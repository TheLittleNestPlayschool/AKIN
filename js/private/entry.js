...
export function setupPrivateEntry({publicMode,privateMode}){
  const orb=publicMode.orb.orb,layer=makeUnlockLayer(),copy=layer.querySelector("#unlockCopy"),body=layer.querySelector("#unlockBody"),actions=layer.querySelector("#unlockActions");let tapCount=0,tapTimer=null,pointerStart=null,gesturePoints=[],holdTimer=null,comboTapCount=0,quoteHoldTimer=null,entryTriggered=false;
  ...
  function showUnlock(){
    const c=config();if(!c?.onboardingComplete)return;entryTriggered=true;publicMode.orb.hideQuote();layer.classList.add("is-visible");layer.setAttribute("aria-hidden","false");copy.textContent="Enter your PIN.";let pin="";
  ...
  orb.addEventListener("click",()=>{
    const c=config();if(entryTriggered){entryTriggered=false;return}if(!c?.onboardingComplete){normalTap();return}
  ...
  orb.addEventListener("pointerdown",event=>{
    const c=config();if(!c?.onboardingComplete)return;entryTriggered=false;pointerStart={x:event.clientX,y:event.clientY};gesturePoints=[event.clientX];
  ...
  function finishPointer(){const c=config();window.clearTimeout(holdTimer);if(c?.method==="gesture"&&!entryTriggered&&gesturePoints.length>3){const start=gesturePoints[0],min=Math.min(...gesturePoints),max=Math.max(...gesturePoints),end=gesturePoints[gesturePoints.length-1];if(start-min>22&&max-min>55&&max-end>22)showUnlock()}pointerStart=null;gesturePoints=[]}
